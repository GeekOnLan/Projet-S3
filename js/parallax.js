...
var Parallax = function(nbLayer) {
	...
	for(var i = 0; i < nbLayer; i++){
		...
		this.tabLayer[i].css('margin-left', "-10px");
		this.tabLayer[i].css('padding', "0px 10px 10px 10px");
	}
...
Parallax.prototype = {
	...
	moveLayer	: function(e) {
		for(var i = 1; i < this.tabLayer.length; i++){
			var mouseX = (e.pageX - ($(window).width() / 2)) * 0.010 * Math.abs(i-this.tabLayer.length);
			var mouseY = (e.pageY - ($(window).height()/ 2)) * 0.005 * Math.abs(i-this.tabLayer.length);
